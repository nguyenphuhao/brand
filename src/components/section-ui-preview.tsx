  ...
  return (
    <div className="bg-brand-dark rounded-card-lg p-4 text-white w-72 shrink-0">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {[
          { icon: "🏸", name: "Cầu lông", count: 12, bg: "bg-brand-primary/20" },
          { icon: "⚽", name: "Bóng đá", count: 8, bg: "bg-brand-accent/20" },
        ].map((s) => (
...
function MockBooking() {
  return (
    <div className="bg-white rounded-card-lg p-4 w-72 shrink-0 border border-gray-100">
      <p className="text-xs font-semibold uppercase tracking-wider text-brand-primary mb-3">Đặt sân</p>
      <div className="space-y-3">
        {[
          ...
          <div key={s.name} className="border border-gray-100 rounded-btn p-3">
            ...
            <p className="text-xs text-brand-dark/50 mt-1">{s.time} · {s.price}</p>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="bg-white rounded-card-lg p-4 w-72 shrink-0 border border-gray-100">
      ...
      <div className="space-y-2">
        {[
          { name: "Minh Tuấn", rank: "#1", badge: "🥇" },
          { name: "Hoàng Anh", rank: "#2", badge: "🥈" },
          { name: "Thu Hà", rank: "#3", badge: "🥉" },
        ].map((m) => (
          <div key={m.name} className="flex items-center gap-3 py-2 border-b border-gray-50">
            <div className="w-8 h-8 rounded-full bg-brand-primary/10 flex items-center justify-center text-sm">
              {m.badge}
            </div>
            <div>
  ...
  return (
    <div className="bg-brand-dark rounded-card-lg p-4 w-72 shrink-0">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-8 h-8 rounded-full bg-brand-ai flex items-center justify-center text-sm">🤖</div>

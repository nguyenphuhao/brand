...
export function SectionColors() {
  return (
    <section id="colors" className="py-24 px-6 bg-brand-light">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-16">
          {palette.map((c) => (
            <div key={c.hex} className="space-y-2">
              ...
              <p className="text-xs text-brand-dark/60">{c.usage}</p>
            </div>
          ))}
        </div>
        <h3 className="text-xl font-bold text-brand-dark mb-6">Semantic Colors</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {semanticColors.map((c) => (
            <div key={c.name} className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-sm text-brand-dark">{c.name}</p>
                <p className="font-mono text-xs text-brand-dark/50">{c.hex}</p>
              </div>
            </div>
          ))}
        </div>
        <h3 className="text-xl font-bold text-brand-dark mb-6">Dark Mode</h3>
